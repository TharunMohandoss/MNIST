...
class Visualizer:
	...
	def Visualize(self,data_dict,order,type_list,cur_epoch,accumulate):
		top_folder = os.path.join(self.config.visualization_dir,self.mode)
		directories_to_create = [top_folder]
		top_folder = os.path.join(self.config.visualization_dir,self.mode,str(cur_epoch))
		directories_to_create = [top_folder]
		i = 0
		for i in range(len(order)):
			...
			if current_type == "satimage10m":
				directories_to_create.append(os.path.join(top_folder,element))
				for i in range(len(self.config.bands_list_10m)):
					directories_to_create.append(os.path.join(top_folder,element,self.config.bands_list_10m[i]))
			if current_type == "satimage20m":
				directories_to_create.append(os.path.join(top_folder,element))
				for i in range(len(self.config.bands_list_20m)):
					directories_to_create.append(os.path.join(top_folder,element,self.config.bands_list_20m[i]))
			if current_type == "imglist":
				directories_to_create.append(os.path.join(top_folder,element))
				for i in range(6):
					directories_to_create.append(os.path.join(top_folder,element,str(i)))
		create_dirs(directories_to_create)


		file = open(os.path.join(top_folder,"visualize.html"),'w')
		starting_text = "<html><head></head><body><table border=\"1\"><tr><th>Sl</th>"
		...
		for i in range(len(data_dict[order[0]])):
			...
			for j in range(len(order)):
				...
				if current_type == "image":
					...
					cv2.imwrite(file_name,255.0*np.transpose(data[0:3],(1,2,0)))
					...
					string_to_write = "<table border=\"1\"><tr>"
					bands_html_list = ""
					images_html_list = ""
					...
					self.html_text += string_to_write
				elif current_type == "satimage10m":
					string_to_write = "<table border=\"1\"><tr>"
					bands_html_list = ""
					images_html_list = ""
					for k in range(len(self.config.bands_list_10m)):
						bands_html_list += ('<td>'+self.config.bands_list_10m[k]+'</td>')
						file_name = os.path.join(top_folder,element,self.config.bands_list_10m[k],str(cur_i)+".jpg")
						images_html_list += ('<td>'+'<img src="./'+str(element)+"/"+self.config.bands_list_10m[k]+"/"+str(cur_i)+'.jpg" alt="Girl in a jacket" style="width:'+str(data.shape[2])+'px;height:'+str(data.shape[1])+'px;"></img>'+'</td>')
						cv2.imwrite(file_name,255*data[k])
					string_to_write += bands_html_list + "</tr><tr>"+images_html_list+"</tr></table>"
					self.html_text += string_to_write
				elif current_type == "satimage20m":
					string_to_write = "<table border=\"1\"><tr>"
					bands_html_list = ""
					images_html_list = ""
					for k in range(len(self.config.bands_list_20m)):
						bands_html_list += ('<td>'+self.config.bands_list_20m[k]+'</td>')
						file_name = os.path.join(top_folder,element,self.config.bands_list_20m[k],str(cur_i)+".jpg")
						images_html_list += ('<td>'+'<img src="./'+str(element)+"/"+self.config.bands_list_20m[k]+"/"+str(cur_i)+'.jpg" alt="Girl in a jacket" style="width:'+str(256)+'px;height:'+str(256)+'px;"></img>'+'</td>')
						cv2.imwrite(file_name,255*np.swapaxes(data[k],0,1))
					string_to_write += bands_html_list + "</tr><tr>"+images_html_list+"</tr></table>"
					self.html_text += string_to_write
				elif current_type == "imglist":
					# self.html_text+='ac'
					string_to_write = "<table border=\"1\"><tr>"
					bands_html_list = ""
					images_html_list = ""
					for k in range(len(data)):
						bands_html_list += ('<td>'+str(k)+'</td>')
						file_name = os.path.join(top_folder,element,str(k),str(cur_i)+".jpg")
						images_html_list += ('<td>'+'<img src="./'+str(element)+"/"+str(k)+"/"+str(cur_i)+'.jpg" alt="Girl in a jacket" style="width:'+str(data.shape[2])+'px;height:'+str(data.shape[3])+'px;"></img>'+'</td>')
						cv2.imwrite(file_name,255.0*np.transpose(data[k],(1,2,0)))
