import numpy as np
import sys
from utils.Visualizer_sift import Visualizer
import time
...
class Trainer():
    ...
    def train(self):
        ...
        for cur_epoch in range(self.config.num_epochs):
            ...
            print(' -------------------------------')
            if(cur_epoch == 320):
                self.config.boost_generator_1()
                self.config.visualization_frequency = 5
            elif(cur_epoch == 640):
                self.config.boost_generator_2()
                self.config.visualization_frequency = 1
    ...
    def train_epoch(self,cur_epoch):
        num_steps = len(self.datagen)
        accumulated_results  = dict()
        start_time_epoch = time.time()
        #for i in range(num_steps):            
        cur_step = 0
        i = 0
        #for images,classes in tqdm(self.datagen):
        for images,classes in self.datagen:
            start_time = time.time()
            if cur_step == self.config.num_batches-1 and cur_epoch % self.config.visualization_frequency == 0:
                visualize = True
            else:
                visualize = False

            results_step,data_batch = self.model.run_batch(images,classes,"train",cur_epoch,cur_step,visualize)
            ...
            #if((cur_epoch%self.config.visualization_once_in_x_epochs)==0 and (i%self.config.visualize_at_cntr)==0):
#                 print('viz time : ',time.time()-start_t)
        ...
        #if((cur_epoch%self.config.visualization_frequency)==0):
        if(visualize == True):
            start_t = time.time()
